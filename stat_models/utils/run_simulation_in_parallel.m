function [estimation_result] = run_simulation_in_parallel(...
    simulation_name, num_workers, simulation_count, dgp_model, assumed_model, true_params, params_opt_set, ...
    trajectory_length, estimation_method, solver, num_handles ...
    )
    if num_workers > 1
        pool = gcp("nocreate");
        if isempty(pool)
        ...
        for worker_idx = num_workers:-1:1
            ...
            future_results(worker_idx) = parfeval(@make_estimation_simulation, ...
                1, simulation_name, worker_idx, num_workers, ceil(simulation_count / num_workers), dgp_model, assumed_model, true_params, ...
                params_opt_set, trajectory_length, estimation_method, ...
        ...
        estimation_result = make_estimation_simulation( ...
            simulation_name, 1, 1, simulation_count, dgp_model, assumed_model, true_params, ...
            params_opt_set, trajectory_length, estimation_method, ...
            solver, result_handler, num_handles ...
        );
    end
