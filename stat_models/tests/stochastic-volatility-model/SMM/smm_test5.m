...
trajectory_lengths = [250, 500, 1000, 2000];
...
tic
    ...
    for length_idx = 1:size(trajectory_lengths, 2)
        ...
        for idx = 1:size(true_params, 1)
            ...
            sim_name = "rossi_params_24m" + "_" + idx + "_T=" + trajectory_length;
            tic
                disp("start " + sim_name)
...
save("summary_table_24m", "summary_table");
...
tic
    ...
    for length_idx = 1:size(trajectory_lengths, 2)
        ...
        for idx = 1:size(true_params, 1)
            params = true_params(idx, :);   
            sim_name = "rossi_params_34m" + "_" + idx + "_T=" + trajectory_length;
            tic
                disp("start " + sim_name)
...
summary_table = EstimationSimulationResult.aggregate_results(simulations);
save("summary_table_34m", "summary_table");
